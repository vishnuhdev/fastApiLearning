...
message_route = APIRouter(prefix="/messages", tags=["Message"])


@message_route.post('/create',summary="send new message", status_code=201)
async def create_message(request: MessageRequest, db: Depends = Depends(get_db)):
    service = MessageService(db)
    return service.create_message(request)


@message_route.get('/{chat_id}/message', status_code=200, response_model=List[MessageResponse])
